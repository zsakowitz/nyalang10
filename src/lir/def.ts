...
export type Expr =
  ...
  | { k: T.Extern; v: { ty: Type; data: unknown } } // constructs a `T.Extern` by some mechanism specific to the compiler or interpreter used
  | { k: T.ArrayFill; v: { el: Type; len: number } } // evaluates `el`, then constructs a `[typeof el; len]` with the given length, where every element is `el`
  | { k: T.ArrayMap; v: { idx: Id; el: Expr; ret: { el: Type; len: number } } } // constructs an array of type `[ret.el; len]` by evaluating `v.el` with `index` bound to 0, 1, 2, ..., `len-1`
  ...
  | { k: T.IfElse; v: { condition: Expr; if: Expr; else: Expr } } // `condition` must be a `bool`; returns `if` if `condition` evaluates `true`, and `false` otherwise. only one branch is ever evaluated
  ...
  | { k: T.UnionMatch; v: { target: Expr; data: Id; arms: Expr[] } } // there must be as many `arms` as variants in the type of `target`, and `target` must be a union. finds the arm with index of the currently active variant, binds the union's data to `data`, and returns the matched arm.
  // control flow
